...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    print("---AGES AT DEATH | BIRTH-DEATH DATE---")
    for key, value in people_dict.items():
        name = value[NAME_INDEX]
        birth_year = value[BIRTH_YEAR_INDEX]
        death_year = value[DEATH_YEAR_INDEX]
        age_at_death = death_year - birth_year
        print(f"{name} {age_at_death} | {birth_year}-{death_year}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key, value in marriages_dict.items():
        ...
        husband_age_at_wedding = wedding_year - people_dict[husband_key][2]

        wife_name = people_dict[wife_key][0]
        wife_age_at_wedding = wedding_year - people_dict[wife_key][2]
